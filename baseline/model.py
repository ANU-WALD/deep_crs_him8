...
for comb in itertools.combinations(["8", "14"], 2):
    print("Starting", comb)
    a = np.load("/data/sat_precip/b{}_30.npy".format(comb[0]))[:300].flatten().reshape(-1,1)
    a = (a - a.min()) / (a.max() - a.min())
    b = np.load("/data/sat_precip/b{}_30.npy".format(comb[1]))[:300].flatten().reshape(-1,1)
    b = (b - b.min()) / (b.max() - b.min())

    x = np.concatenate((a, b), axis=1)
    print(x.shape)

    y = np.load("/data/sat_precip/crsflux_30.npy")[:300].flatten().reshape(-1,1)
    x_train = x[:35000000,:]
    y_train = y[:35000000,:]
    x_test = x[35000000:,:]
    y_test = y[35000000:,:]
